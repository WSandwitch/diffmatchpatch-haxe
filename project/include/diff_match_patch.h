...
class diff_match_patch {
  ...
  * {Diff(Operation.DELETE_, "Hello"), Diff(Operation.INSERT_, "Goodbye"),
  *  Diff(Operation.EQUAL_, " world.")}
  * which means: delete "Hello", add "Goodbye" and keep " world."
  */
  enum Operation {
    DELETE_=-1, INSERT_=1, EQUAL_=0
  };
  ...
  */
  class Diff {
   public:
    Operation operation;
    // One of: INSERT_, DELETE_ or EQUAL_.
     ...
     * @param operation One of INSERT_, DELETE_ or EQUAL_.
    ...
    static string_t strOperation(Operation op) {
      switch (op) {
        case INSERT_:
          return traits::cs(L"INSERT_");
        case DELETE_:
          return traits::cs(L"DELETE_");
        case EQUAL_:
          return traits::cs(L"EQUAL_");
      }
 ...
 private:
  static void diff_main(const string_t &text1, const string_t &text2, bool checklines, clock_t deadline, Diffs& diffs) {
    ...
    if (text1 == text2) {
      if (!text1.empty()) {
        diffs.push_back(Diff(EQUAL_, text1));
      }
    }
    else {
      ...
      if (!commonprefix.empty()) {
        diffs.push_front(Diff(EQUAL_, commonprefix));
      }
      if (!commonsuffix.empty()) {
        diffs.push_back(Diff(EQUAL_, commonsuffix));
      }
 ...
 private:
  static void diff_compute(string_t text1, string_t text2, bool checklines, clock_t deadline, Diffs& diffs) {
    if (text1.empty()) {
      ...
      diffs.push_back(Diff(INSERT_, text2));
      return;
    }

    if (text2.empty()) {
      // Just delete some text (speedup).
      diffs.push_back(Diff(DELETE_, text1));
      return;
    }

    {
      ...
      if (i != string_t::npos) {
        ...
        const Operation op = (text1.length() > text2.length()) ? DELETE_ : INSERT_;
        diffs.push_back(Diff(op, longtext.substr(0, i)));
        diffs.push_back(Diff(EQUAL_, shorttext));
    ...
    if (deadline != std::numeric_limits<clock_t>::max()) {
      ...
      if (diff_halfMatch(text1, text2, hm)) {
        ...
        diffs.push_back(Diff(EQUAL_, hm.mid_common));
        Diffs diffs_b;
 ...
 private:
  static void diff_lineMode(string_t text1, string_t text2, clock_t deadline, Diffs& diffs) {
    ...
    diffs.push_back(Diff(EQUAL_, string_t()));
    int count_delete = 0;
    int count_insert = 0;
  ...
  static void diff_cleanupSemantic(Diffs &diffs) {
    ...
    for (cur_diff = diffs.begin(); cur_diff != diffs.end();) {
      if ((*cur_diff).operation == EQUAL_) {
        ...
        if (!lastequality.empty()
          ...
          (*(cur_diff = equalities.back())).operation = INSERT_;
          // Duplicate record.
          diffs.insert(cur_diff, Diff(DELETE_, lastequality));
    ...
    if ((cur_diff = diffs.begin()) != diffs.end()) {
      for (typename Diffs::iterator prev_diff = cur_diff; ++cur_diff != diffs.end(); prev_diff = cur_diff) {
        if ((*prev_diff).operation == DELETE_ &&
            (*cur_diff).operation == INSERT_) {
          ...
          if (overlap_length1 >= overlap_length2) {
            if (overlap_length1 >= deletion.size() / 2.0 ||
              ...
              diffs.insert(cur_diff, Diff(EQUAL_, insertion.substr(0, overlap_length1)));
              prev_diff->text =
            ...
            if (overlap_length2 >= deletion.length() / 2.0 ||
              ...
              diffs.insert(cur_diff, Diff(EQUAL_, deletion.substr(0, overlap_length2)));
              prev_diff->operation = INSERT_;
              prev_diff->text =
                  insertion.substr(0, insertion.length() - overlap_length2);
              cur_diff->operation = DELETE_;
 ...
 public:
  static void diff_cleanupSemanticLossless(Diffs &diffs) {
    ...
    for (typename Diffs::iterator next_diff = cur_diff; ++next_diff != diffs.end(); prev_diff = cur_diff, cur_diff = next_diff) {
      if ((*prev_diff).operation == EQUAL_ &&
        (*next_diff).operation == EQUAL_) {
 ...
 public:
  static void diff_cleanupMerge(Diffs &diffs) {
    diffs.push_back(Diff(EQUAL_, string_t()));  // Add a dummy entry at the end.
        ...
        case EQUAL_:
          if (count_delete + count_insert > 1) {
            ...
            if (count_delete != 0 && count_insert != 0) {
              ...
              if (commonlength != 0) {
                if (cur_diff != diffs.begin()) {
                  ...
                  diffs.insert(cur_diff, Diff(EQUAL_, text_insert.substr(0, commonlength)));
                }
            ...
            if (!text_delete.empty()) {
              diffs.insert(cur_diff, Diff(DELETE_, text_delete));
            }
            if (!text_insert.empty()) {
              diffs.insert(cur_diff, Diff(INSERT_, text_insert));
            }
 ...
 public:
  static int diff_xIndex(const Diffs &diffs, int loc) {
    ...
    if (last_diff != diffs.end() && (*last_diff).operation == DELETE_) {
    ...
    for (typename string_t::const_pointer token = delta.c_str(); token - delta.c_str() < (int)delta.length(); token += token_len + 1) {
      ...
      switch (traits::to_wchar(*token)) {
        case L'+':
          ...
          diffs.push_back(Diff(INSERT_, param));
          break;
        case L'-':
          // Fall through.
        case L'=': {
          ...
          if (traits::to_wchar(*token) == L'=') {
            diffs.push_back(Diff(EQUAL_, text));
          } else {
            diffs.push_back(Diff(DELETE_, text));
          }
          break;
        }
        default:
 ...
 protected:
  void patch_addContext(Patch &patch, const string_t &text) const {
    ...
    if (!prefix.empty()) {
      patch.diffs.push_front(Diff(EQUAL_, prefix));
    }
    // Add the suffix.
    ...
    if (!suffix.empty()) {
      patch.diffs.push_back(Diff(EQUAL_, suffix));
    }
 ...
 public:
  Patches patch_make(const string_t &text1, const Diffs &diffs) const {
    ...
    if (!diffs.empty()) { // Get rid of the null case.
      ...
      for (typename Diffs::const_iterator cur_diff = diffs.begin(); cur_diff != diffs.end(); ++cur_diff) {
        if (patch.diffs.empty() && (*cur_diff).operation != EQUAL_) {
          ...
          if ((int)text1.length() > Match_MaxBits
            ...
            for (typename Diffs::const_iterator cur_diff = (*cur_patch).diffs.begin(); cur_diff != (*cur_patch).diffs.end(); ++cur_diff) {
              if ((*cur_diff).operation != EQUAL_) {
                ...
                } else if ((*cur_diff).operation == DELETE_) {
                  // Deletion
 ...
 public:
  string_t patch_addPadding(Patches &patches) const {
    ...
    if (firstPatchDiffs.empty() || firstPatchDiffs.front().operation != EQUAL_) {
      // Add nullPadding equality.
      firstPatchDiffs.push_front(Diff(EQUAL_, nullPadding));
    ...
    if (lastPatchDiffs.empty() || lastPatchDiffs.back().operation != EQUAL_) {
      // Add nullPadding equality.
      lastPatchDiffs.push_back(Diff(EQUAL_, nullPadding));
      ...
      while (!bigpatch.diffs.empty()) {
        ...
        while (!bigpatch.diffs.empty()
          ...
          if (diff_type == INSERT_) {
            ...
            empty = false;
          } else if (diff_type == DELETE_ && patch.diffs.size() == 1
              && patch.diffs.front().operation == EQUAL_
        ...
        if (!postcontext.empty()) {
          ...
          if (!patch.diffs.empty()
              && patch.diffs.back().operation == EQUAL_) {
            patch.diffs.back().text += postcontext;
          } else {
            patch.diffs.push_back(Diff(EQUAL_, postcontext));
          }
        }
        if (!empty) {
      ...
      while (text - textline.c_str() < (int)textline.length()) {
        ...
        for (text += text_len + 1; text - textline.c_str() < (int)textline.length(); text += text_len + 1) {
          ...
          switch (traits::to_wchar(sign)) {
            case L'-':
              // Deletion.
              patch.diffs.push_back(Diff(DELETE_, line));
              continue;
            case L'+':
              // Insertion.
              patch.diffs.push_back(Diff(INSERT_, line));
              continue;
            case L' ':
              // Minor equality.
              patch.diffs.push_back(Diff(EQUAL_, line));
              continue;
            case L'@':
              // Start of next patch.
              break;
